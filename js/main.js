...
const scrollFn = () => {
    ...
    const $leftside = document.getElementById('leftside') || null;
    const throttledScroll = utils.throttle((e) => {
        ...
        if (currentTop > 0) {
            ...
            $leftside && ($leftside.style.cssText = 'opacity: 0.8; transform: translate(0, -70px);');
        } else {
            $header.classList.remove('nav-fixed', 'nav-visible');
            $leftside && ($leftside.style.cssText = "opacity: ''; transform: ''");
        }
    }, 200);
...
const percent = () => {
    ...
    document.querySelectorAll(".leftsideEndHide").forEach(item => item.classList.toggle("hide", totalScrollableHeight - scrollPos < 100));
}
const showTodayCard = () => {
    ...
    musicToggle(isMeting = true) {
        ...
        if (this.musicPlaying) {
            if (typeof rm !== 'undefined' && rm?.menuItems.music[0]) {
                ...
                $rmIcon.className = 'solitude st-pause-fill';
                }
        } else {
                if (typeof rm !== 'undefined' && rm?.menuItems.music[0]) {
                $rmText.textContent = GLOBAL_CONFIG.right_menu.music.start;
                $rmIcon.className = 'solitude st-play-fill';
            }
        }
